 ...
 * CellGroup会参与遗传和进化，但是它生成的细胞不会参与遗传。 各个Group生成的细胞相加总和就是脑细胞总数。
 * 
 * Group在脑活动中不起作用，可以把Group比作播种机，把种子排列好后，就撒手不管了,在遗传过程中有一个fat参数，如果细胞活跃多，则保留及变异的可能性大，反之则舍弃掉。
 * Group是器官的一种，所以蛋里存放着所有Group的位置、大小、内部参数等信息，但是蛋里面不保存具体的细胞。这样通过控制有多少个"播种机"，就可以控制大脑的结构了。
 * Group可以进行位置、大小变动、复制、分裂等多种形式的变异,原则上Group遗传的一下代与父代是同一个播种类型，但不排除也有可能突变成另一个类型的Group。
 * 
 * @author Yong Zhu
 * @since 1.0
 */
