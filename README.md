...
* EGG_QTY: 每次允许Frog下多少个蛋，通常取值在10~1000之间。蛋保存着我们测试的结果。实验的最终目标就是获得一个蛋。
* FOOD_QTY：食物的数量，食物越多，则Frog的生存率就越高，能量排名靠前的一批Frog可以下蛋，其余的被淘汰。    
另外每步演示的结果(egg)会存盘在根目根目录下，名为egg.ser，可以删除这个文件以从头开始新的测试。下面是这个测试的屏幕截图，有兴趣的可以试着自己运行一下:  
[截图](result.gif)
